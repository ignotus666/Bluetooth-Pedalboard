...
void wah()
{
  ...
  wahRead = analogRead(A0);
  wahVal = wahRead;                  //Assign A0 value to wahVal.
  wahBar = wahRead;
  wahPerc = wahRead;

  wahVal = map(wahVal, 303, 480, 0, 127);   //Map the range of the hall sensor to a range of 0-127 for MIDI.
  wahBar = map(wahBar, 303, 480, 0, 78);    //Map sensor range to 0-78 for arc.
  wahPerc = map(wahPerc, 303, 480, 0, 100); //Map sensor range to 0-100 for percentage.
